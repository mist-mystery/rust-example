...
//! - ライフタイムパラメータの指定は、ライフタイム省略規則(lifetime elision rules)により省略できることがある。
//! - non-lexical lifetime (NLL)とは、ライフタイムの region の推論をレキシカルスコープベースでなく制御フローグラフ(control-flow graph)ベースにするもの。  
